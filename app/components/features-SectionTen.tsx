import React from "react";
import { useLanguage } from "../../contexts/language-context";

export default function FeaturesSectionTen() {
  const { t, isRTL } = useLanguage();

  return (
    <div className="mt-32 mx-auto max-w-sm  sm:max-w-xl md:max-w-2xl lg:max-w-4xl xl:max-w-6xl">
      <div
        dir={isRTL ? "rtl" : "ltr"}
        className="grid md:grid-cols-3 grid-cols-1 gap-8 mx-auto"
      >
        <div className="mx-auto">
          <h1 className="font-semibold  text-xl">
            {t("hostingPlans.virtualServersTitle")}
          </h1>
          <p className="mt-2 text-sm">
            {t("hostingPlans.virtualServersDescription")}
          </p>
        </div>
        <div className="mx-auto">
          <div className="flex items-center gap-2">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_9_78481)">
                <path
                  d="M12 0C5.37262 0 0 5.37262 0 12C0 18.6277 5.37262 24 12 24C18.6277 24 24 18.6277 24 12C24 5.37262 18.6277 0 12 0ZM12 22.5236C6.21037 22.5236 1.5 17.7896 1.5 12C1.5 6.21033 6.21037 1.49995 12 1.49995C17.7896 1.49995 22.5 6.21035 22.5 12C22.5 17.7896 17.7896 22.5236 12 22.5236ZM16.7891 7.60913L9.74848 14.694L6.57785 11.5234C6.28498 11.2305 5.81023 11.2305 5.51698 11.5234C5.2241 11.8162 5.2241 12.291 5.51698 12.5839L9.2291 16.2964C9.52198 16.5889 9.99673 16.5889 10.29 16.2964C10.3237 16.2626 10.3526 16.2259 10.3789 16.1876L17.8504 8.66998C18.1429 8.3771 18.1429 7.90235 17.8504 7.60913C17.5571 7.31625 17.0824 7.31625 16.7891 7.60913Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0_9_78481">
                  <rect width="24" height="24" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <p>{t("hostingPlans.highPerformance")}</p>
          </div>
          <div className="flex items-center gap-2 md:mt-6 mt-2">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_9_78481)">
                <path
                  d="M12 0C5.37262 0 0 5.37262 0 12C0 18.6277 5.37262 24 12 24C18.6277 24 24 18.6277 24 12C24 5.37262 18.6277 0 12 0ZM12 22.5236C6.21037 22.5236 1.5 17.7896 1.5 12C1.5 6.21033 6.21037 1.49995 12 1.49995C17.7896 1.49995 22.5 6.21035 22.5 12C22.5 17.7896 17.7896 22.5236 12 22.5236ZM16.7891 7.60913L9.74848 14.694L6.57785 11.5234C6.28498 11.2305 5.81023 11.2305 5.51698 11.5234C5.2241 11.8162 5.2241 12.291 5.51698 12.5839L9.2291 16.2964C9.52198 16.5889 9.99673 16.5889 10.29 16.2964C10.3237 16.2626 10.3526 16.2259 10.3789 16.1876L17.8504 8.66998C18.1429 8.3771 18.1429 7.90235 17.8504 7.60913C17.5571 7.31625 17.0824 7.31625 16.7891 7.60913Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0_9_78481">
                  <rect width="24" height="24" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <p>{t("hostingPlans.enterpriseReliability")}</p>
          </div>
        </div>
        <div className="mx-auto">
          <div className="flex items-center gap-2 ">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_9_78481)">
                <path
                  d="M12 0C5.37262 0 0 5.37262 0 12C0 18.6277 5.37262 24 12 24C18.6277 24 24 18.6277 24 12C24 5.37262 18.6277 0 12 0ZM12 22.5236C6.21037 22.5236 1.5 17.7896 1.5 12C1.5 6.21033 6.21037 1.49995 12 1.49995C17.7896 1.49995 22.5 6.21035 22.5 12C22.5 17.7896 17.7896 22.5236 12 22.5236ZM16.7891 7.60913L9.74848 14.694L6.57785 11.5234C6.28498 11.2305 5.81023 11.2305 5.51698 11.5234C5.2241 11.8162 5.2241 12.291 5.51698 12.5839L9.2291 16.2964C9.52198 16.5889 9.99673 16.5889 10.29 16.2964C10.3237 16.2626 10.3526 16.2259 10.3789 16.1876L17.8504 8.66998C18.1429 8.3771 18.1429 7.90235 17.8504 7.60913C17.5571 7.31625 17.0824 7.31625 16.7891 7.60913Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0_9_78481">
                  <rect width="24" height="24" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <p>{t("hostingPlans.uptimeGuarantee")}</p>
          </div>
          <div className="flex items-center gap-2 md:mt-6 mt-2">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_9_78481)">
                <path
                  d="M12 0C5.37262 0 0 5.37262 0 12C0 18.6277 5.37262 24 12 24C18.6277 24 24 18.6277 24 12C24 5.37262 18.6277 0 12 0ZM12 22.5236C6.21037 22.5236 1.5 17.7896 1.5 12C1.5 6.21033 6.21037 1.49995 12 1.49995C17.7896 1.49995 22.5 6.21035 22.5 12C22.5 17.7896 17.7896 22.5236 12 22.5236ZM16.7891 7.60913L9.74848 14.694L6.57785 11.5234C6.28498 11.2305 5.81023 11.2305 5.51698 11.5234C5.2241 11.8162 5.2241 12.291 5.51698 12.5839L9.2291 16.2964C9.52198 16.5889 9.99673 16.5889 10.29 16.2964C10.3237 16.2626 10.3526 16.2259 10.3789 16.1876L17.8504 8.66998C18.1429 8.3771 18.1429 7.90235 17.8504 7.60913C17.5571 7.31625 17.0824 7.31625 16.7891 7.60913Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0_9_78481">
                  <rect width="24" height="24" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <p>{t("hostingPlans.nvmeSsd")}</p>
          </div>
        </div>
      </div>
    </div>
  );
}
